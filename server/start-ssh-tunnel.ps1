# SSH tunnel via plink (password from server/.env)
param(
    [switch]$Quiet,
    [switch]$Background
)
...
$pass = $cfg.Password
if (-not $pass) {
    Write-Host '[ERROR] SSH_PASSWORD missing in server/.env' -ForegroundColor Red
    exit 1
}
...
$plink = Find-Plink
if (-not $plink) {
    Write-Host '[ERROR] plink.exe not found. Install PuTTY.' -ForegroundColor Red
    Write-Host '        Or use putty-tunnel-guide.txt (manual tunnel)' -ForegroundColor Yellow
    exit 1
}
...
if (-not $Quiet) {
    ...
    Write-Host '  Method: plink + password from server/.env'
    Write-Host '============================================'
    Write-Host ''
}
...
if ($Background) {
    ...
    if (-not $Quiet) {
        ...
        Write-Host '  Check SSH_PASSWORD or use putty-tunnel-guide.txt' -ForegroundColor Yellow
    }
