...
--
-- Процент фиксируется при броне, но купон списывается только при УСПЕШНОЙ
-- покупке (coupon_held=FALSE для новых броней). Пока бронь активна, процент
-- не перекручивается. Игрок не узнаёт процент до завершения покупки.
-- Legacy coupon_held=TRUE: купон уже был списан при броне — при expire/release
