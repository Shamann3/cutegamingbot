...
// ---------------------------------------------------------------------------
// Tab: Salaries (management — senior/owner)
// ---------------------------------------------------------------------------
...
function SalariesTab({ isOwner, canPay = false }) {
  ...
  const handleRemind = async () => {
    ...
    try {
      ...
      alert(r.sent ? 'Напоминание отправлено владельцам' : (r.detail || 'Нечего напоминать'))
    } catch (err) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const data = await fetchStaffSalaries()
  ...
  const handleSet = async (userId) => {
    ...
    if (!Number.isFinite(baseAmount) || baseAmount < 0) { alert('Введите ставку'); return }
    setBusy(`set-${userId}`)
    try {
      await setStaffSalary({
        ...
        payoutType: dft.payoutType || 'other',
      })
      await load()
    } catch (err) {
      alert(err?.message || 'Ошибка')
  ...
  const handleResolve = async (appealId) => {
    const resolution = prompt('Решение по апелляции (необязательно):') ?? ''
    setBusy(`appeal-${appealId}`)
    try {
      await resolveSalaryAppeal(appealId, resolution)
      await load()
    } catch (err) {
      alert(err?.message || 'Ошибка')
    } finally {
  ...
  return (
    <div className="sec-tab-body">
      <p className="staff-hint">
        Зарплата на неделю {week ? `(с ${fmtDate(week + 'T00:00:00')})` : ''}.
        Если работник не согласен он может подать апелляцию. Завышенная зарплата
        может быть снята, всё пересматривается.
      </p>

      <div className="staff-debts-banner">
        <span className="staff-badge" style={{ '--badge-color': '#fbbf24' }}>⏳ ждут одобрения: {pendingCount}</span>
        <span className="staff-badge" style={{ '--badge-color': '#fb923c' }}>💸 не выплачено: {unpaidCount}</span>
        <span className="staff-badge" style={{ '--badge-color': '#94a3b8' }}>➖ без начисления: {notSetCount}</span>
      </div>

      <div className="sec-audit-filters">
        <button className="sec-btn sec-btn-ghost" onClick={load}>Обновить</button>
        {isOwner && (
          <button className="sec-btn sec-btn-ghost" disabled={busy === 'remind'} onClick={handleRemind}>
      ...
      <div className="staff-members">
        {rows.map((m) => {
          const s = m.salary
          const st = s ? SALARY_STATUS[s.status] : null
          const dft = drafts[m.userId] || {}
          const locked = s && s.status === 'paid'
          return (
            <div key={m.userId} className="staff-salary-card-row">
              <div className="staff-member-info">
                <span className="staff-card-name">{nameOf(m)}</span>
                <span className="staff-badge" style={{ '--badge-color': ROLE_BADGE_COLOR[m.role] || '#94a3b8' }}>
                  {roleLabel(m.role)}
                ...
                {s && s.status === 'partially_paid' && (
                  <span className="staff-card-date">выплачено {s.paidAmount ?? 0} / {s.amount}</span>
                )}
                {s?.txid && <span className="staff-card-date">TXID: {s.txid}</span>}
              </div>
              ...
              <input className="sec-input" placeholder="за что бонус (необязательно)" disabled={locked}
                value={dft.bonusReason ?? ''} onChange={(e) => setField(m.userId, 'bonusReason', e.target.value)} />
              <input className="sec-input" placeholder="за что штраф (необязательно)" disabled={locked}
                value={dft.penaltyReason ?? ''} onChange={(e) => setField(m.userId, 'penaltyReason', e.target.value)} />

              <div className="staff-payslip-total">
                Итого к выплате: <b>{draftTotal(dft)}</b>
                <span className="staff-card-date">
                  = {Number.parseInt(dft.base, 10) || 0} × {Number.parseFloat(dft.coefficient) || 0}
                  {' '}+ {Number.parseInt(dft.bonus, 10) || 0} − {Number.parseInt(dft.penalty, 10) || 0}
                </span>
              </div>

              <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.82rem', color: '#94a3b8' }}>
                Способ выплаты:
                <AdminSelect
                  value={dft.payoutType || 'other'}
              ...
              <div className="staff-salary-actions">
                {!locked && (
                  <button className="sec-btn sec-btn-sm" disabled={busy === `set-${m.userId}`} onClick={() => handleSet(m.userId)}>
                    {s ? 'Пересохранить' : 'Выставить'}
                  </button>
                )}
                {isOwner && s && s.status === 'pending_approval' && (
                  <button className="sec-btn sec-btn-sm" disabled={busy === `appr-${s.salaryId}`}
                ...
                {s && s.status !== 'paid' && s.status !== 'cancelled' && (
                  <button className="sec-btn sec-btn-sm" disabled={busy === `cancel-${s.salaryId}`}
                    onClick={() => {
                      if (confirm('Снять (аннулировать) начисление?')) {
        ...
        {!loading && rows.length === 0 && (
          <p className="sec-empty">Нет сотрудников для начисления</p>
        )}
      </div>
      ...
      {appeals.length > 0 && (
        <div className="staff-appeals">
          ...
          {appeals.map((a) => (
            <div key={a.appealId} className="staff-member-row">
              <div className="staff-member-info">
                <span className="staff-card-name">{nameOf(a)}</span>
                <span className="staff-badge" style={{ '--badge-color': '#fbbf24' }}>
                  зарплата {a.amount}
                </span>
              </div>
              <div className="staff-member-meta">
                <span className="staff-answer-a">{a.reason}</span>
              </div>
              <button
                className="sec-btn sec-btn-ghost sec-btn-sm"
                disabled={busy === `appeal-${a.appealId}`}
                onClick={() => handleResolve(a.appealId)}
              >
                Рассмотреть
              </button>
            </div>
    ...
    <div className="sec-tab-body">
      ...
      {current ? (
        <div className="staff-salary-card">
          <p className="staff-salary-card-label">Ваша зарплата на эту неделю</p>
          <p className="staff-salary-card-amount"><CountUp value={current.amount} /></p>
          <span className="staff-badge" style={{ '--badge-color': (SALARY_STATUS[current.status] || {}).color || '#94a3b8' }}>
            {(SALARY_STATUS[current.status] || {}).label || current.status}
          </span>
          ...
          {/* Кнопка получить kut */}
          {current.status === 'approved' && current.payoutType === 'kut' && (
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  await load()
                  alert(`✅ Получено ${r.amount} kut на игровой баланс!`)
                } catch (err) {
                  alert(err?.message || 'Ошибка')
                } finally {
                  setBusy(false)
            ...
            >
              {busy ? '…' : `🟡 Получить ${current.amount} kut`}
            </button>
          )}

          {!current.appealId && current.status !== 'paid' && current.payoutType !== 'kut' && (
            <button className="sec-btn sec-btn-sm" onClick={() => setAppealFor(current)}>
              Подать апелляцию
            </button>
          )}
          {current.appealStatus === 'open' && (
            <p className="staff-hint">Апелляция подана, ожидает рассмотрения.</p>
          )}
        </div>
      ) : (
        !loading && <p className="sec-empty">Зарплата на эту неделю ещё не назначена</p>
      )}
      ...
      {items.length > 1 && (
        <div className="staff-members">
          ...
          {items.slice(1).map((s) => (
            <div key={s.salaryId} className="staff-member-row">
              <div className="staff-member-info">
                <span className="staff-card-name">{fmtDate(s.weekStart + 'T00:00:00')}</span>
                <span className="staff-badge" style={{ '--badge-color': (SALARY_STATUS[s.status] || {}).color || '#94a3b8' }}>
                  {s.amount} · {(SALARY_STATUS[s.status] || {}).label || s.status}
                </span>
              </div>
            </div>
          ))}
      ...
      {appealFor && (
        <div className="admin-modal-backdrop" role="presentation" onClick={() => !busy && setAppealFor(null)}>
          <div className="admin-modal" role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
            <h3 className="admin-modal-title">Апелляция по зарплате</h3>
            <p className="admin-modal-desc">Зарплата: {appealFor.amount}</p>
            <label className="admin-modal-field">
              ...
              <textarea
                ...
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder="Почему вы не согласны с зарплатой"
                disabled={busy}
              />
            </label>
            <div className="admin-modal-actions">
              <button className="panel-users-btn" disabled={busy} onClick={() => setAppealFor(null)}>
                Отмена
              </button>
              <button className="panel-users-btn panel-users-btn-primary" disabled={busy || !reason.trim()} onClick={submitAppeal}>
                {busy ? '…' : 'Отправить'}
              </button>
            </div>
          </div>
