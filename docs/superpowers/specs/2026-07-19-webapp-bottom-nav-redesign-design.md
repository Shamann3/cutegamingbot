...
- **Продать** — карточка «Продать из рюкзака» + `MarketSellModal`, без изменений.

Отдельного блока «Мои лоты» не нужно: в `MarketShelfTile.jsx` у элементов каталога уже есть флаг `item.isMine` (бейдж «Мой» + класс `market-shelf-tile-mine`), и `MarketListingModal.jsx` уже показывает кнопку «Снять с биржи» вместо «Купить», когда `item.isMine` — то есть управление своими лотами уже работает внутри сегмента «Биржа» сегодня, ничего добавлять не нужно.
