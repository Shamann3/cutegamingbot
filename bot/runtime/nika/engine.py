...
async def _raise_empty_ladder(
    ...
    *,
    ...
    title = (
        ...
        if dry
        else "Технические группы пусты — доливать стол не из чего"
    )
    body = (
        ...
        "Код уже обошёл всю лестницу и нигде не нашёл кут. "
        "Сам он эту дыру закрыть не может — нужны кнопка долива, пауза или пополнение техгрупп."
    )
    async with db.pool.acquire() as conn:
...
async def _alert_empty_ladder(db, bot, settings: Dict[str, Any], group_chat_id: int, need: int, snapshot: Dict[int, int]) -> None:
    ...
    if not user_id or bot is None:
        return
    lines = [
        "Ника: технические группы пусты, пополнить стол нечем.",
        f"Группа {group_chat_id}: не хватает ещё {need} кут.",
        "",
        _ladder_lines(snapshot),
        "",
        "Это серьёзно: игроки продолжат играть, а доливать стол не из чего. Смотри вкладку Ника в админке.",
    ]
    try:
