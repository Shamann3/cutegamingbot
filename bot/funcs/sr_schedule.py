...
Режимы:
  • interval — каждые N секунд (после initial_delay с старта процесса)
  • hourly   — каждый час в :MM (timezone)
  • times    — в выбранные HH:MM по дням недели (timezone)
"""
...
DEFAULT_CFG: Dict[str, Any] = {
    ...
    "hourly_minute": 0,
    "daily_times": ["03:00"],
    "weekdays": [0, 1, 2, 3, 4, 5, 6],  # Mon=0 … Sun=6
...
def normalize_config(raw: Optional[Dict[str, Any]], *, defaults: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict):
        for k in list(base.keys()):
            if k in raw:
                base[k] = raw[k]
        # allow unknown forward-compatible keys? keep closed to known
...
def compute_next_at(
    ...
    *,
    ...
    """Unix timestamp следующего авто-рестарта, или None если авто выкл."""
    ...
    if mode == "interval":
        ...
        # следующий интервал от «сейчас» (после неудачи) или от earliest
        return now + float(cfg["interval_sec"])

    # wall-clock modes: не раньше earliest
    ...
    # times
    times = [_parse_hhmm(t) for t in cfg["daily_times"]]
    times = [t for t in times if t]
    weekdays = set(cfg["weekdays"])
    if not times:
        times = [(3, 0)]
    # ищем в пределах 8 дней
...
def preview_next_runs(
    ...
    *,
    ...
    """Следующие N запусков для UI (приблизительно)."""
    import time as _time
    ...
    for _ in range(max(1, min(24, count))):
        ...
        if out and nxt <= out[-1]["at"]:
            # продвинуть время, чтобы не зациклиться
            t = nxt + 1.0
            first = False
        ...
        t = nxt + 1.0
        first = False
        # для interval после «виртуального» рестарта процесс как будто стартует заново
