...
@dp.message(lambda message: message.text.lower() in ["мои рефы","мои приграшенные","мой реф", "мои инвайты","мои инваиты","мой реф", "снять флаг" , "Снять флаг" , "Флаг снять" , "флаг снять ","sypherддать","sypherдддать","sypherсснять","sypherссснять","sypheraddgift","sypherupdategift","рулетка хелп" , "хелп рулетка" ,
                                                                                    "помощь рулетка" ,
                                                                                    "рулетка помощь" ,
                                                                                    "рулетка инструкция" ,
                                                                                    "инструкция рулетка" ,
...
async def balance(message: Message):
    ...
    # ========== ОБРАБОТКА КОМАНДЫ "МОИ РЕФЕРАЛЫ" ==========
    phrases1 = [ "мои рефы" , "мои приграшенные" ,  # сохранено как в запросе
        "мой реф" , "мои инвайты" , "мои инваиты" , "мой реф"  # дублируется, но оставлено как есть
    ]



    if message.text and " ".join((message.text or "").lower().strip().split()) in phrases1:
        user_id = message.from_user.id

        try:
            # Получаем список приглашённых пользователей из БД
            referrals = await db.get_referrals_with_details(user_id)

            if not referrals:
                await message.answer(
                    "<tg-emoji emoji-id='5402495686392431549'>🤷‍♂️</tg-emoji> <b>У вас пока нет приглашённых пользователей.</b>" ,
                    parse_mode="HTML")
                return

            # Формируем список с нумерацией и жирными именами
            lines = [ ]
            for idx , ref in enumerate(referrals , start=1):
                first_name = ref.get('first_name') or 'Пользователь'
                username = ref.get('username')

                # Экранируем спецсимволы HTML
                safe_name = first_name.replace('&' , '&amp;').replace('<' , '&lt;').replace('>' , '&gt;')

                # Создаём кликабельную ссылку с жирным именем
                if username:
                    link = f"<b>@{username}</b>"
                else:
                    link = f"<a href='tg://user?id={ref [ 'user_id' ]}'><b>{safe_name}</b></a>"

                # Добавляем номер
                lines.append(f"{idx}. {link}")

            count = len(lines)
            win_amount_formatted = "{:,.0f}".format(count).replace("," , ".")
            answer_text = (
                    f"<tg-emoji emoji-id='5348582551063641258'>😌</tg-emoji> <b>Ваши рефералы [ {win_amount_formatted} ] :\n\n</b>" + "\n".join(
                lines) + f"\n\n<blockquote><b><tg-emoji emoji-id='5472401690793614752'>🛍️</tg-emoji> В случае каких либо проблем. Пишите в @CuteGamingSupportBot</b></blockquote>")

            await message.answer(answer_text , parse_mode="HTML" , disable_web_page_preview=True)

        except Exception as e:
            logger.error(f"Ошибка при получении рефералов для {user_id}: {e}")
            await message.answer(
                "<tg-emoji emoji-id='5213205860498549992'>⚠️</tg-emoji> <b>Произошла ошибка. Попробуйте позже.</b>" ,
                parse_mode="HTML")
    ...
    if message.text and " ".join((message.text or "").lower().strip().split()) in [ "рулетка хелп" , "хелп рулетка" ,
        ...
        await message.reply(
            (f"<tg-emoji emoji-id='5321499578216769477'>🎩</tg-emoji> <b>Рулетка</b>\n"
             ...
             f"(редкий удар, около 3% — колесо маленькое, 0…12)\n"
    ...
    if message.text and " ".join((message.text or "").lower().strip().split()) in [ "футбол хелп" , "хелп футбол" ,
        ...
        await message.reply(
            (f"<tg-emoji emoji-id='5188361519095649349'>⚽️</tg-emoji> <b>Игра «Футбол»</b>\n"
             ...
             f"<tg-emoji emoji-id='5188361519095649349'>⚽️</tg-emoji> <b>Гол</b> - мяч в сетке (анимация Telegram, значения {target_values_text}). Штанга - это мимо.\n"
