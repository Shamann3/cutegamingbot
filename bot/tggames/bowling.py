...
async def tgbowling(message: Message):
            ...
            if pay > 0:
                try:
                    ...
                    gfund_result = await apply_commission(
                        db, bot1, chat_id=chat_id, user_id=user_id, game="bowling", pot=pay,
                    )
                    if gfund_result:
                        pay = max(0, pay - gfund_result["commission"])
                except Exception as e:
                    _bdbg("GFUND", f"apply_commission(0demo_mask) error: {e}")
    ...
    if using_demo:
        ...
        if pay > 0:
            try:
                ...
                _bdbg("GFUND", f"apply_commission(demo) error: {e}")
    ...
    if is_strike:
        ...
        if chat_balance < profit_int:
            ...
            if pay > 0:
                try:
                    ...
                    _bdbg("GFUND", f"apply_commission(partial) error: {e}")

            if has_assignment and pay > 0:
                try:
                    await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=pay, outcome="+")
                    _bdbg("GC", f"WIN PARTIAL gc +{pay}")
                except Exception as e:
            ...
            if gfund_result:
                from bot.funcs.growth_fund import build_commission_button
                kb_rows.append([build_commission_button(gfund_result)])
            await _safe_edit_reply_markup(bowling, InlineKeyboardMarkup(inline_keyboard=kb_rows))
            return
        ...
        await db.update_user_wins(user_id, 1, bot1, ref_coin)
        await db.cutehistory_plus(user_id, float(bet_int + net_profit), "+ боулинг")
