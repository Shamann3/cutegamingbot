...
async def _load_gc_state_for_user_bowling(user_id: int) -> dict:
    ...
    try:
        ...
        _bdbg("GC_LIMIT", f"gc_get_bet_limit_for_user -> {gc_bet_limit!r}")
    except Exception as e:
        gc_bet_limit = None
        _bdbg("GC_LIMIT", f"error: {e}")
    ...
    if gc_bet_limit is not None:
        try:
            ...
            _bdbg("GC_LIMIT", f"convert error {gc_bet_limit!r}: {e}")
    ...
    if assignment and str(assignment.get("status") or "").lower() == "active":
        ...
        _bdbg("GC_STATE", f"user={user_id} active=1 free={is_free} two={current_two} target={target_amount} max_bet={max_bet}")
    else:
        _bdbg("GC_STATE", f"user={user_id} no active assignment max_bet={max_bet}")
...
async def tgbowling(message: Message):
    ...
    max_bet_gc = int(gc_state.get("max_bet") or bowling_BASE_MAX_BET)
