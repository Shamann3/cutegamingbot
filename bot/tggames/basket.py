...
async def _load_gc_state_for_user_basket(user_id: int) -> dict:
    ...
    try:
        ...
        gc_bet_limit = None
        print(f"[GBL][basket] gc limit: {e!r}")
    ...
    if assignment and str(assignment.get("status") or "").lower() == "active":
        ...
        _kdbg("GC_STATE", f"user={user_id} active=1 free={is_free} two={current_two} target={target_amount} gc_lim={gc_bet_limit}")
    else:
        _kdbg("GC_STATE", f"user={user_id} no active assignment gc_lim={gc_bet_limit}")
...
async def tgbasket(message: Message):
    ...
    if gate.get("mode") == "reject":
        await message.reply(format_game_max_bet_html(gate.get("max") or basket_BASE_MAX_BET), parse_mode="HTML")
        return
    ...
    try:
        ...
        print(f"[GBL] basket check skip: {_gbl_e!r}")
        ...
        if pay > 0:
            try:
                ...
                gfund_result = await apply_commission(
                    db, bot1, chat_id=chat_id, user_id=user_id, game="basket", pot=pay,
                )
                if gfund_result:
    ...
    if is_hit:
        ...
        if chat_balance < win_amount_int:
            ...
            if gfund_result:
                from bot.funcs.growth_fund import build_commission_button
                kb_rows.append([build_commission_button(gfund_result)])
            await _safe_edit_reply_markup(basketball, InlineKeyboardMarkup(inline_keyboard=kb_rows))
            return
        ...
        if profit_int > 0:
            try:
                from bot.funcs.growth_fund import apply_commission
                gfund_result = await apply_commission(
                    db, bot1, chat_id=chat_id, user_id=user_id, game="basket", pot=profit_int,
                )
                if gfund_result:
        ...
        await db.update_user_wins(user_id, 1, bot1, ref_coin)
        await db.cutehistory_plus(user_id, float(bet_int + net_profit), "+ баскетбол")
