...
async def _load_gc_state_for_user_soccer(user_id: int) -> dict:
    ...
    try:
        ...
        gc_bet_limit = None
        print(f"[GBL][soccer] gc limit: {e!r}")
    ...
    if assignment and str(assignment.get("status") or "").lower() == "active":
        ...
        _sdbg("GC_STATE", f"user={user_id} active=1 free={is_free} two={current_two} target={target_amount} gc_lim={gc_bet_limit}")
    else:
...
async def tgsoccer(message: Message):
    ...
    if gate.get("mode") == "reject":
        await message.reply(format_game_max_bet_html(gate.get("max") or soccer_BASE_MAX_BET), parse_mode="HTML")
        return
    ...
    try:
        ...
        print(f"[GBL] soccer check skip: {_gbl_e!r}")
    ...
    if is_goal:
        ...
        if profit_int > 0:
            try:
                from bot.funcs.growth_fund import apply_commission
                gfund_result = await apply_commission(
                    db, bot1, chat_id=chat_id, user_id=user_id, game="soccer", pot=profit_int,
                )
                if gfund_result:
        ...
        await db.update_user_wins(user_id, 1, bot1, ref_coin)
        await db.cutehistory_plus(user_id, float(bet_int + net_profit), "+ футбол")
