# -*- coding: utf-8 -*-
"""
🎯 Дартс (дартс/дарт) (ставка) - полная версия с Jericho, маскировкой и сериями.
... (описание режимов)
"""
...
last_darts_time: Dict[int, Dict[int, float]] = LazyGameStore("last_darts_time")
_darts_streaks: Dict[int, Dict[str, int]] = LazyGameStore("darts_streaks")

DARTS_DEBUG = True

# Эмодзи для кнопок
BAD_THROW_ICON_ID = "5890815115552362075"   # 🎯 (неудачный бросок)
LOSS_ICON_ID = "4956499161319998529"        # 💥 (промах)
WIN_ICON_ID = "5310278924616356636"         # 🎯 (попадание)

# Эмодзи для демо (гарантированное попадание)
DEMO_DARTS_EMOJIS = [
    "<tg-emoji emoji-id='5891181334528789506'>🎯</tg-emoji>"
]

# Эмодзи для 0demo (проигрыш)
ZERO_DEMO_DARTS_EMOJIS = [
    "<tg-emoji emoji-id='5891075592433962392'>🎯</tg-emoji>",
    "<tg-emoji emoji-id='5890981489700506545'>🎯</tg-emoji>",
    "<tg-emoji emoji-id='5890896071390924854'>🎯</tg-emoji>",
    "<tg-emoji emoji-id='5888919733599735824'>🎯</tg-emoji>",
    "<tg-emoji emoji-id='5891052704553242318'>🎯</tg-emoji>",
]

# Маскировочные механики
DEMO_MASK_LOSS_PROB = 0.45               # вероятность подмены WIN → LOSS в demo-режиме
DEMO_MASK_BAD_PROB = 0.05                # вероятность подмены WIN → BAD_THROW в demo-режиме
...
# Целевые значения dice, означающие попадание (центр)
DARTS_TARGET_VALUES = {6}

# Вероятность «неудачного броска»
try:
    DARTS_BAD_THROW_CHANCE = DARTS_BAD_THROW_CHANCE
except NameError:
    DARTS_BAD_THROW_CHANCE = Decimal("0.03")

# Вероятность промаха (не 6)
DARTS_MISS_PROBABILITY = Decimal(5) / Decimal(6)

# Условная вероятность неудачного броска при промахе
if DARTS_MISS_PROBABILITY > 0:
    DARTS_BAD_CONDITIONAL = DARTS_BAD_THROW_CHANCE / DARTS_MISS_PROBABILITY
else:
    DARTS_BAD_CONDITIONAL = Decimal(0)


# ===================== DEBUG =====================
def _ddbg(tag: str, msg: str) -> None:
    if not DARTS_DEBUG:
        return
    try:
        print(f"[DARTS][{tag}] {msg}", flush=True)
    except Exception:
        pass


def _ddbg_err(tag: str, err: Exception) -> None:
    if not DARTS_DEBUG:
        return
    try:
        print(f"[DARTS][ERROR][{tag}] {err}", flush=True)
...
def _is_bad_throw_roll_conditional() -> bool:
...
async def _mark_user_game_activity(user_id: int, reason: str = "") -> None:
    try:
        ...
        _ddbg("ACTIVITY", f"touch ok user_id={user_id} reason={reason}")
    except Exception as e:
        _ddbg("ACTIVITY", f"touch error user_id={user_id} reason={reason}: {e}")
...
async def _send_invoice_later(message: Message, user_id: int, stars_amount: str, delay: float) -> None:
    try:
        ...
        if ctx and not ctx.get("sent"):
            ...
            _ddbg("DONATE", f"invoice sent user_id={user_id} stars={stars_amount}")
    except Exception as e:
        _ddbg("DONATE", f"invoice later error user_id={user_id}: {e}")
...
def _cooldown_left(chat_id: int, user_id: int) -> int:
    ...
    chat_map = last_darts_time.setdefault(chat_id, {})
...
def _cooldown_mark(chat_id: int, user_id: int) -> None:
    ...
    _save_safe(last_darts_time)


def _get_streaks(user_id: int) -> Dict[str, int]:
    streaks = _darts_streaks.get(user_id)
    if streaks is None:
        streaks = {"win_streak": 0, "lose_streak": 0}
        _darts_streaks[user_id] = streaks
        _save_safe(_darts_streaks)
    return streaks
...
def _update_streaks(user_id: int, is_win: bool, is_bad: bool = False) -> None:
    ...
    _darts_streaks[user_id] = streaks
    _save_safe(_darts_streaks)
    _ddbg("STREAKS", f"user={user_id} win={streaks['win_streak']} lose={streaks['lose_streak']}")
...
# ===================== ЛОГ "ДОМОЙ" =====================
async def _home_take_and_log_darts_bad_throw(*, user_id: int, loss: int) -> None:
    """Списание в TECH_CHAT при неудачном броске."""
    try:
        ...
        chat_balance = await db.get_chat_balance(bot1, TECH_CHAT_ID)

        # HTML-эмодзи - единственный текст сообщения
        emoji_html = '<tg-emoji emoji-id="5350460637182993292">🎯</tg-emoji>'
        ...
        await safe_send_tech_log(
            ...
            tag="HOME_BAD_THROW_LOG_SEND",
        )

    except Exception as e:
        _ddbg_err("HOME_BAD_THROW_LOG", e)


# ===================== GC / ЧЕЛЛЕНДЖ =====================
async def _load_gc_state_for_user_darts(user_id: int) -> dict:
    ...
    try:
        ...
        _ddbg("GC_STATE", f"get_active_gc_assignment -> {bool(assignment)}")
    except Exception as e:
        assignment = None
        _ddbg("GC_STATE", f"error: {e}")
    ...
    if assignment and str(assignment.get("status") or "").lower() == "active":
        ...
        try:
            ...
            is_free = False
            _ddbg("GC_STATE", f"error gc_active_is_free: {e}")
        ...
        try:
            ...
            current_two = 0
            _ddbg("GC_STATE", f"error gc_get_current_two_balance: {e}")
...
async def _tgdarts_free_game(
    ...
    if left > 0:
        ...
        _ddbg("CD", f"free cooldown block left={left}s")
        return
    ...
    if bet_int > current_two:
        ...
        _ddbg("FREE", f"not enough two_balance bet={bet_int} two={current_two} target={target_amount}")
        return
    ...
    mult_dec = _dec(multiplier_darts)
    bet_dec = _dec(bet_int)

    if bad_throw_now:
        try:
            await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=bet_int, outcome="-")
            _ddbg("GC", f"FREE BAD_THROW gc -{bet_int}")
        except Exception as e:
            _ddbg("GC", f"FREE BAD_THROW gc error: {e}\n{traceback.format_exc()}")

        await _mark_user_game_activity(user_id, reason="bad_throw_free")
        ...
        button = InlineKeyboardButton(
            ...
            style="primary",
            icon_custom_emoji_id=BAD_THROW_ICON_ID
        )
        await _safe_edit_reply_markup(dart, InlineKeyboardMarkup(inline_keyboard=[[button]]))
        return
    ...
    if is_hit:
        ...
        if profit_int > 0:
            try:
                ...
                _ddbg("GC", f"FREE WIN gc +{profit_int}")
            except Exception as e:
                _ddbg("GC", f"FREE WIN gc error: {e}\n{traceback.format_exc()}")
    ...
    try:
        await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=bet_int, outcome="-")
        _ddbg("GC", f"FREE LOSS gc -{bet_int}")
    except Exception as e:
        _ddbg("GC", f"FREE LOSS gc error: {e}\n{traceback.format_exc()}")
...
async def tgdarts(message: Message):
    ...
    if len(parts) != 2:
        await message.reply(
            "<tg-emoji emoji-id='6028346797368283073'>✈️</tg-emoji> <b>Формат: дартс (ставка)</b>",
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
        _ddbg("FMT", f"bad format text={message.text!r}")
        return
    ...
    try:
        if await db.get_newbie_expires_at(user_id) is None:
            ...
            _ddbg("NEWBIE", f"newbie expires set {expires}")
    except Exception as e:
        _ddbg("NEWBIE", f"error {e}")
    ...
    if not has_assignment:
        try:
            ...
            _ddbg("DEMO/0DEMO", f"error: {e}")
            demo_balance = 0
            zero_demo_balance = 0

        _ddbg("BONUS", f"demo={demo_balance} 0demo={zero_demo_balance} bet={bet_int}")

        # Jericho
        _ddbg("JERICHO", f"calling jericho_check user={user_id} bet={bet_int}")
        decision = await jericho_check(user_id, bet_int, game_name="дартс")
        jericho_action = decision.get("action", "normal")
        _ddbg("JERICHO", f"action={jericho_action} reason={decision.get('reason','')}")
        ...
        if demo_enough and zero_enough:
            if demo_balance > zero_demo_balance:
                using_demo = True
                _ddbg("BONUS", "base: demo (больше)")
            elif zero_demo_balance > demo_balance:
                using_0demo = True
                _ddbg("BONUS", "base: 0demo (больше)")
            else:
                using_demo = True
                _ddbg("BONUS", "base: demo (равны)")
        elif demo_enough:
            using_demo = True
            _ddbg("BONUS", "base: demo (только demo)")
        elif zero_enough:
            using_0demo = True
            _ddbg("BONUS", "base: 0demo (только 0demo)")
        else:
            _ddbg("BONUS", "base: обычный")
        ...
        if jericho_action in win_actions:
            if demo_enough:
                using_demo = True
                using_0demo = False
                _ddbg("JERICHO", f"{jericho_action} → switched to demo")
            else:
                _ddbg("JERICHO", f"{jericho_action} but demo insufficient → keep base")
        elif jericho_action in loss_actions:
            if zero_enough:
                using_0demo = True
                using_demo = False
                _ddbg("JERICHO", f"{jericho_action} → switched to 0demo")
            else:
                _ddbg("JERICHO", f"{jericho_action} but 0demo insufficient → keep base")
        else:
            _ddbg("JERICHO", "no relevant action → keep base")
    ...
    if not using_demo and not using_0demo and bet_int > balance:
        ...
        _ddbg("DONATE", f"need buy bet={bet_int} stars={stars_amount} bot=@{bot_username}")
        return
    ...
    if left > 0:
        ...
        _ddbg("CD", f"cooldown block left={left}s")
        return
    _cooldown_mark(chat_id, user_id)
    ...
    if using_0demo:
        ...
        if lose_streak >= ZERO_STREAK_BREAK:
            should_win = True
            _ddbg("0DEMO_MASK", f"streak break: lose_streak={lose_streak} -> WIN")
        elif random.random() < ZERO_MASK_WIN_PROB:
            should_win = True
            _ddbg("0DEMO_MASK", "random chance -> WIN")

        # Выбираем начальное эмодзи в зависимости от ожидаемого исхода
        if should_win:
            initial_emoji = random.choice(DEMO_DARTS_EMOJIS)   # выигрышное
        else:
            initial_emoji = random.choice(ZERO_DEMO_DARTS_EMOJIS)  # проигрышное
        ...
        bad_throw_now = False
        if not should_win:
            bad_throw_now = _is_bad_throw_roll_raw()
        ...
        if should_win:
            ...
            if pay > 0:
                ...
                try:
                    await db.cutehistory_plus(user_id, float(pay), "+ дартс (0demo маскировка)")
                except Exception as e:
                    _ddbg("HISTORY", f"cutehistory_plus(0demo_mask) error: {e}")
                try:
                    await db.update_user_wins(user_id, 1, bot1, ref_coin)
                except Exception as e:
                    _ddbg("STATS", f"update_user_wins(0demo_mask) error: {e}")

            if has_assignment and profit_int > 0:
                try:
                    await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=profit_int, outcome="+")
                    _ddbg("GC", f"0DEMO_MASK WIN gc +{profit_int}")
                except Exception as e:
                    _ddbg("GC", f"0DEMO_MASK WIN gc error: {e}")
        ...
        # Обычный проигрыш: списываем 0demo, гасим долг
        try:
            await db.deduct_0demo_amount(user_id, bet_int)
            _ddbg("0DEMO", f"deduct 0demo {bet_int}")
        except Exception as e:
            _ddbg("0DEMO", f"deduct 0demo error: {e}")

        _ddbg("0DEMO_DEBT", f"calling force_repay_debt({user_id}, {bet_int})")
        try:
            await force_repay_debt(user_id, bet_int)
            _ddbg("0DEMO_DEBT", f"force_repay_debt completed for {user_id} amount {bet_int}")
        except Exception as e:
            _ddbg_err("0DEMO_DEBT_FAILED", e)

        if has_assignment:
            try:
                await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=bet_int, outcome="-")
                _ddbg("GC", f"0DEMO LOSS gc -{bet_int}")
            except Exception as e:
                _ddbg("GC", f"0DEMO LOSS gc error: {e}")

        ok_minus = await _user_minus(user_id, bet_int)
        _ddbg("0DEMO_LOSS", f"user_minus={ok_minus} amount={bet_int}")

        try:
            await db.cutehistory_minus(user_id, bet_int, "- дартс (0demo)")
        except Exception as e:
            _ddbg("HISTORY", f"cutehistory_minus(0demo) error: {e}")
        try:
            await db.update_user_loose(user_id, 1, bot1, ref_coin)
            await db.update_game_last_activity(user_id)
        except Exception as e:
            _ddbg("STATS", f"update_user_loose(0demo) error: {e}")

        await _mark_user_game_activity(user_id, reason="0demo_loss")
        await _safe_add_xp(user_id)
        _update_streaks(user_id, is_win=False, is_bad=bad_throw_now)

        if bad_throw_now:
            await _home_take_and_log_darts_bad_throw(user_id=user_id, loss=bet_int)
            button = InlineKeyboardButton(
                text="Неудачный бросок",
                callback_data="money_won",
                style="primary",
    ...
    if using_demo:
        ...
        if win_streak >= DEMO_STREAK_BREAK:
            should_lose = True
            _ddbg("DEMO_MASK", f"streak break: win_streak={win_streak} -> LOSS")
        else:
            r = random.random()
            if r < DEMO_MASK_BAD_PROB:
                should_bad = True
                _ddbg("DEMO_MASK", "random chance -> BAD_THROW")
            elif r < DEMO_MASK_BAD_PROB + DEMO_MASK_LOSS_PROB:
                should_lose = True
                _ddbg("DEMO_MASK", "random chance -> LOSS")
        ...
        if should_lose or should_bad:
            ...
            if has_assignment:
                try:
                    await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=bet_int, outcome="-")
                    _ddbg("GC", f"DEMO_MASK LOSS gc -{bet_int}")
                except Exception as e:
                    _ddbg("GC", f"DEMO_MASK LOSS gc error: {e}")
        ...
        if pay > 0:
            try:
                ...
                _ddbg("GFUND", f"apply_commission(demo) error: {e}")

        if pay > 0:
            await _chat_minus(chat_id, pay)
            await _user_plus(user_id, pay)
            try:
                await db.cutehistory_plus(user_id, float(pay), "+ дартс")
            except Exception as e:
                _ddbg("HISTORY", f"cutehistory_plus(demo) error: {e}")
            try:
                await db.update_user_wins(user_id, 1, bot1, ref_coin)
            except Exception as e:
                _ddbg("STATS", f"update_user_wins(demo) error: {e}")

        if has_assignment and pay > 0:
            try:
                await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=pay, outcome="+")
                _ddbg("GC", f"DEMO WIN gc +{pay}")
            except Exception as e:
                _ddbg("GC", f"DEMO WIN gc error: {e}")
    ...
    if bad_throw_now:
        if has_assignment:
            try:
                await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=bet_int, outcome="-")
                _ddbg("GC", f"BAD_THROW gc -{bet_int}")
            except Exception as e:
                _ddbg("GC", f"BAD_THROW gc error: {e}\n{traceback.format_exc()}")

        ok_minus = await _user_minus(user_id, bet_int)
        _ddbg("BAD_THROW", f"user_minus={ok_minus} amount={bet_int}")

        try:
            await db.cutehistory_minus(user_id, float(bet_int), "- дартс (неудачный бросок)")
        except Exception as e:
            _ddbg("HISTORY", f"cutehistory_minus(bad_throw) error: {e}")
        try:
            await db.update_user_loose(user_id, 1, bot1, ref_coin)
            await db.update_game_last_activity(user_id)
        except Exception as e:
            _ddbg("STATS", f"update_user_loose(bad_throw) error: {e}")

        await _mark_user_game_activity(user_id, reason="bad_throw")
        await _safe_add_xp(user_id)
    ...
    if is_hit:
        ...
        if balance < bet_int:
            ...
            _ddbg("RACE", f"balance dropped before payout balance={balance} bet={bet_int}")
            return

        win_amount = (bet_dec * mult_dec).quantize(Decimal("1"), rounding=ROUND_HALF_UP)
        win_amount_int = int(win_amount)
        profit_int = max(0, win_amount_int - bet_int)
        _ddbg("WIN", f"win_amount={win_amount_int} profit={profit_int}")
        ...
        if chat_balance < win_amount_int:
            ...
            if pay > 0:
                try:
                    ...
                    _ddbg("GFUND", f"apply_commission(partial) error: {e}")
            ...
            if has_assignment and effective_profit > 0:
                try:
                    ...
                    _ddbg("GC", f"WIN PARTIAL gc +{effective_profit}")
                except Exception as e:
                    _ddbg("GC", f"WIN PARTIAL gc error: {e}\n{traceback.format_exc()}")
            ...
            btn = InlineKeyboardButton(
                text="Нажми на меня",
                callback_data=f"errordarts_{pay}",
                style="default",
            ...
            if gfund_result:
                from bot.funcs.growth_fund import build_commission_button
                kb_rows.append([build_commission_button(gfund_result)])
            await _safe_edit_reply_markup(dart, InlineKeyboardMarkup(inline_keyboard=kb_rows))
            return
        ...
        button = InlineKeyboardButton(
            text=f"{_fmt_int(net_profit)} кут | {mult_dec:.1f}x",
            callback_data="money_won",
            style="default",
            icon_custom_emoji_id=WIN_ICON_ID
        )
    ...
    if has_assignment:
        try:
            await gc_process_bet(user_id=user_id, event_chat_id=chat_id, bet=bet_int, outcome="-")
            _ddbg("GC", f"LOSS gc -{bet_int}")
        except Exception as e:
            _ddbg("GC", f"LOSS gc error: {e}\n{traceback.format_exc()}")

    await _user_minus(user_id, bet_int)
    await _chat_plus(chat_id, bet_int)
    await db.update_user_loose(user_id, 1, bot1, ref_coin)
    await db.cutehistory_minus(user_id, float(bet_int), "- дартс")
...
# ===================== CALLBACKS =====================
@dp.callback_query(F.data.startswith("errordarts_"))
async def errordarts_alert(callback_query: CallbackQuery):
...
async def darts_info_stub(callback_query: CallbackQuery):
    try:
