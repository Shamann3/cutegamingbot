...
async def _load_gc_state_for_user_slots(user_id: int) -> dict:
    ...
    try:
        ...
        gc_bet_limit = None
        print(f"[GBL][slots] gc limit: {e!r}")
    ...
    if assignment and str(assignment.get("status") or "").lower() == "active":
        ...
        _sdbg(
            ...
            f"target={target_amount} gc_lim={gc_bet_limit}"
        )
    else:
        _sdbg("GC_STATE", f"user={user_id} no active assignment gc_lim={gc_bet_limit}")
...
async def tgslots(message: Message):
    ...
    if gate.get("mode") == "reject":
        await message.reply(format_game_max_bet_html(gate.get("max") or slots_BASE_MAX_BET), parse_mode="HTML")
        return
    ...
    try:
        ...
        print(f"[GBL] slots check skip: {_gbl_e!r}")
    ...
    if multiplier > 0:  # WIN
        ...
        if chat_balance < profit_int:
            ...
            # ТОЛЬКО при проигрыше (see JAM-ветку выше) - при выигрыше она у
            # игрока никогда не забиралась. Значит чат физически может быть
            # должен максимум profit_int, а НЕ win_amount_int (bet+profit).
            # Раньше порог сравнивался с win_amount_int, а pay = chat_balance
            # без вычета bet_int - если баланс группы был между profit_int и
