...
Логика переключения:
    DATABASE_MODE = "main"  -> боевая база (правится в bot/config/db.py)
    DATABASE_MODE = "test"  -> тестовая база

    DATABASE_LOCATION = "local"  -> Прямое подключение к DigitalOcean (внешний хост :25060)
    DATABASE_LOCATION = "remote" -> SSH-туннель для локальной разработки (127.0.0.1:15432)

Модуль автоматически читает переменную DOTENV_B64 (с DigitalOcean) и раскрывает её.
"""
...
# --------------------------------------------------------------------------- #
#  Чтение DOTENV_B64 (КЛЮЧЕВОЕ УЛУЧШЕНИЕ!)
# --------------------------------------------------------------------------- #
...
def _load_file_env() -> Dict[str, str]:
    """Слияние: server/.env -> корневой .env -> DOTENV_B64 (приоритет у B64)."""
    merged: Dict[str, str] = {}

    # 1. Читаем обычные файлы
    merged.update(_parse_env_file(SERVER_ENV_FILE))
    merged.update(_parse_env_file(ROOT_ENV_FILE))

    # 2. Читаем DOTENV_B64 поверх (высший приоритет!)
...
# Дефолты для матрицы 2×2 (исправлено под ваш DigitalOcean):
#   local  -> Прямое подключение к DO (внешний хост :25060)
#   remote -> SSH-туннель для локальной разработки (127.0.0.1:15432)
_DB_DEFAULTS = {
    ("test", "local"):  {"host": "db-pgsql-ams3-34361-do-user-18976217-0.k.db.ondigitalocean.com", "port": "25060", "name": "cutebase", "user": "doadmin", "password": "", "ssl": "true"},
    ("main", "local"):  {"host": "db-pgsql-ams3-34361-do-user-18976217-0.k.db.ondigitalocean.com", "port": "25060", "name": "cutebase", "user": "doadmin", "password": "", "ssl": "true"},
    ("test", "remote"): {"host": "127.0.0.1", "port": "15432", "name": "cutebase", "user": "postgres", "password": "", "ssl": "false"},
    ("main", "remote"): {"host": "127.0.0.1", "port": "15432", "name": "cutebase", "user": "postgres", "password": "", "ssl": "false"},
}

# Ищем файл конфигурации (у вас он называется db.py, поэтому путь изменён)
_CONFIG_PY_FILE = Path(__file__).resolve().parent / "db.py"

def _read_config_var(name: str) -> str:
    """Читает DATABASE_MODE / DATABASE_LOCATION из db.py БЕЗ импорта модуля."""
    try:
...
# Значения главных переключателей из db.py (высший приоритет).
...
def _resolve_app_mode() -> str:
    # 1) db.py -> 2) .env (APP_MODE) -> 3) по умолчанию test
    if _CONFIG_PY_MODE:
...
def _resolve_location() -> str:
    """Где база: local (внешний DO) / remote (SSH-туннель)."""
    # 1) db.py
    ...
    # 3) авто: main -> remote (SSH), test -> local (DO)
    return "remote" if APP_MODE == "main" else "local"

# host/port/ssl зависят от РАСПОЛОЖЕНИЯ (local:25060 / remote:15432 через туннель),
...
def app_mode_summary() -> str:
    if DB_LOCATION == "remote":
        ...
        where = "прямое подключение к DigitalOcean"
...
def validate_database_profile() -> List[str]:
    ...
    if DB_LOCATION == "local" and DB_PORT == 15432:
        warnings.append("local на порту :15432 это порт SSH-туннеля. Локально обычно :25060.")
    return warnings
...
def bootstrap_database_env() -> None:
    ...
    # Главный переключатель из db.py имеет приоритет: жёстко выставляем
    ...
    bar = "#" * 64
    loc_word = "ПРЯМОЕ ПОДКЛЮЧЕНИЕ (DO)" if DB_LOCATION == "local" else "СЕРВЕР CuteHost (SSH)"
    if APP_MODE == "main":
    ...
    src_mode = "db.py" if _CONFIG_PY_MODE else ".env"
    src_loc = "db.py" if _CONFIG_PY_LOCATION else ".env/авто"
    _safe_log(f"##  DATABASE_MODE={ACTIVE_DB_PROFILE} (из {src_mode}) | "
              f"DATABASE_LOCATION={DB_LOCATION} (из {src_loc})")
    _safe_log(f"##  сменить: db.py -> DATABASE_MODE и DATABASE_LOCATION")
    _safe_log(bar)
