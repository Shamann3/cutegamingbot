...
async def coupon_possibilities(db, user_id, message):
    ...
    (db.add_demo_amount через bot/funcs/growth_fund.py -> use_ticket) - ЧТО
    ПОЛЕ/исход раунда те же, что и у любого другого источника demo.

    В отличие от обычного demo, ДОПОЛНИТЕЛЬНО ставит в очередь гарантированный
    раунд (use_ticket -> add_coupon_guarantee, списывается в jericho_check,
    main.py) - следующий PvE-раунд детерминированно запускается в льготном
    режиме, а не по обычной вероятностной ветке demo (которая сама может
    решить force_loss несмотря на demo-баланс - без этой гарантии купон был
    неотличим от случайного подарка).

    Текст НЕ называет конкретный процент шанса победы - реальная вероятность
    внутри самого льготного режима своя у каждой игры, а называть цифру,
    которая может не совпасть с реальной механикой, в продукте с реальными
    деньгами нельзя (см. комментарий в bot/config/config.py рядом с
    GROWTH_FUND_TICKET_*).
    """
    ...
    await message.reply(
        "<tg-emoji emoji-id='5433758796289685818'>👑</tg-emoji> <b>Купон Возможностей использован!</b>\n\n"
        "Ваша следующая одиночная игра гарантированно запускается в льготном режиме - "
        "с намного более высокими шансами на победу, чем обычно. Удачи!",
        parse_mode="HTML",
    )
