...
Это измеритель, а не проверка поведения: нужен живой Redis и аргумент командной
строки, а «провалиться» он не может. Поэтому pytest его не собирает - имя файла
не подходит под `python_files = test_*.py` в pytest.ini.
"""
import os
import pathlib
import sys
...
def main():
    ...
    # Флаг читается на импорте pklcode, поэтому выставляем его до импорта.
