...
Проверки против настоящего Redis (2, 4 и хвосты остальных) живут в отдельном
тесте с маркером `redis`; проверки структур в памяти идут всегда.
"""
import os
import pathlib
import sys
...
def main(with_redis: bool = False):
    print(f"redis: {'есть' if with_redis else 'НЕТ (проверки в Redis пропущены)'}")

    # ---------- 1-2. смесь операций, затем проверка на утечки ----------
    s = _fresh(NAME, with_redis)
    s.expiry_seconds = 60
    ...
    s = _fresh("_soak_norm", with_redis)
...
@pytest.mark.slow
def test_soak_in_memory_structures_do_not_grow():
    """Проверки 1, 3, 5, 6: после 90k операций структуры в памяти не разъезжаются."""
...
@pytest.mark.skipif(
    not HAVE_REDIS,
    reason="проверки 2 и 4 сверяют содержимое хешей в живом Redis "
           "и целостность данных после «перезапуска» процесса",
)
def test_soak_redis_stays_in_sync():
    """Тот же прогон целиком, включая сверку с Redis и восстановление после рестарта."""
