...
Проверки 3, 5 и 11 требуют живого Redis и живут в отдельном тесте с маркером
`redis`; остальные полностью в памяти и идут всегда.
"""
import os
import pathlib
import sys
...
def _fresh(name: str, with_redis: bool = False) -> P.GameStore:
    s = P.GameStore(name)
...
def main(with_redis: bool = False):
    assert P.PKL_HOT_PATH_V2, "тест написан под PKL_HOT_PATH_V2=1"
    print(f"redis: {'есть' if with_redis else 'НЕТ (проверки персистентности пропущены)'}")
    ...
    # ---------- 2. чтения не растят _expire_heap ----------
    s = _fresh("_test_hotpath", with_redis)
    s["живой"] = 1
    ...
    # ---------- 3. запись доезжает до Redis и читается обратно ----------
    if with_redis:
        s = _fresh("_test_persist", with_redis)
    ...
    s = _fresh("_test_forever", with_redis)
    ...
    if with_redis:
        s = _fresh("_test_touch", with_redis)
        s.expiry_seconds = 60
        s["горячий"] = "v"
        s.flush()
    ...
    # ---------- 6. запись не создаёт поток на каждую операцию ----------
    s = _fresh("_test_threads", with_redis)
    ...
    # ---------- 7. запись не делает рекурсивное сравнение вложенных структур ----------
    s = _fresh("_test_nested", with_redis)
...
def test_hot_path_invariants_in_memory():
    """Проверки 1, 2, 4, 6-10: горячий путь не ходит в Redis, TTL локальный."""
    main(with_redis=False)


@pytest.mark.redis
@pytest.mark.skipif(
    not HAVE_REDIS,
    reason="проверки 3, 5 и 11 читают и пишут в живой Redis "
           "(персистентность записи, батч touch-меток, восстановление после падения Redis)",
)
def test_hot_path_persistence_with_redis():
    """Тот же сценарий целиком, включая проверки персистентности в Redis."""
    main(with_redis=True)


if __name__ == "__main__":
    sys.stdout.reconfigure(encoding="utf-8")
    main(with_redis=HAVE_REDIS)
